...
export async function GET() {
  try {
    ...
    const bookings: BookingType[] = raw.map(normalizeBooking);
    ...
    if (upcoming.length > 0) {
      ...
      const upRows = [...upcoming]
        ...
        .map((b) => {
          ...
          const daysLeft = Math.ceil((dd.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
          return [
            b.name,
            b.phone,
            b.location,
      ...
      autoTable(doc, {
        startY: y,
        head: [["Farmer", "Phone", "Location", "Tubes", "Delivery Date", "Days Left"]],
        body: upRows,
        theme: "plain",
    ...
    if (overdue.length > 0) {
      ...
      const overdueRows = [...overdue]
        .sort((a, b) => getDeliveryDate(a).getTime() - getDeliveryDate(b).getTime())
        .map((b) => {
          const dd = getDeliveryDate(b);
          const daysLate = Math.floor((today.getTime() - dd.getTime()) / (1000 * 60 * 60 * 24));
          return [
            b.name,
            b.phone,
            b.location,
